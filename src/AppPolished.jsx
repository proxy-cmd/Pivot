...
export default function AppPolished() {
  const input = useRef(); const [data, setData] = useState(null); const [overview, setOverview] = useState(null); const [analyses, setAnalyses] = useState([]); const [selected, setSelected] = useState(null); const [page, setPage] = useState('Overview'); const [busy, setBusy] = useState(false); const [chatBusy, setChatBusy] = useState(false); const [message, setMessage] = useState(''); const [toast, setToast] = useState(''); const [profile, setProfile] = useState(() => ({ ...readJson('pivot-personal-profile'), ...readJson('pivot-auth-session') })); const [authMode, setAuthMode] = useState(null); const [transitioning, setTransitioning] = useState(false); const [authenticated, setAuthenticated] = useState(() => Boolean(localStorage.getItem('pivot-auth-session'))); const [restoring, setRestoring] = useState(() => Boolean(localStorage.getItem('pivot-active-dataset'))); const [question, setQuestion] = useState(''); const [messages, setMessages] = useState([]); const [sql, setSql] = useState('SELECT * FROM dataset LIMIT 20'); const [result, setResult] = useState(null); const [preview, setPreview] = useState(null); const [autoPilot, setAutoPilot] = useState(null)
  ...
  function reset() { localStorage.removeItem('pivot-active-dataset'); setData(null); setOverview(null); setAnalyses([]); setPreview(null); setAutoPilot(null); setMessages([]); setResult(null); setPage('Overview'); setMessage('') }
  ...
  async function ask() { const asked = question.trim(); if (!asked || !data?.dataset_id || chatBusy) return; setQuestion(''); setChatBusy(true); setMessages(current => [...current, { role: 'user', text: asked }, { role: 'assistant', pending: true }]); try { const response = await request('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dataset_id: data.dataset_id, question: asked, context: { page, history: messages.slice(-6).map(item => ({ role: item.role, text: item.text })) } }) }); setMessages(current => [...current.slice(0, -1), { role: 'assistant', text: response.answer, source: response.source, sql: response.sql, query_result: response.query_result, rows: response.rows, insights: response.insights, driver_rows: response.driver_rows, visualization: response.visualization, action: response.action, download_url: response.download_url, citations: response.citations }]); if (response.action?.status === 'approved') { await refresh(data.dataset_id); notify('A new cleaned dataset version is ready to download.') } } catch (error) { setMessages(current => [...current.slice(0, -1), { role: 'assistant', text: error.message, source: 'error' }]) } finally { setChatBusy(false) } }
  ...
  else if (page === 'Auto Pilot') content = <AutoPilot data={data} busy={busy} report={autoPilot} run={runAutoPilot} />
  ...
  else if (page === 'AI Analyst') content = <AnalystV3 data={data} question={question} setQuestion={setQuestion} messages={messages} ask={ask} busy={chatBusy} />
